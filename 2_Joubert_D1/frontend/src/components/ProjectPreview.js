  ...
  return (
    <div className="bg-terminal-bg border border-terminal-border rounded-lg p-5 transition-all duration-300 ease-in-out shadow-[0_0_10px_rgba(0,255,0,0.1)] hover:border-terminal-accent hover:shadow-[0_0_20px_rgba(0,255,0,0.2)] hover:transform hover:-translate-y-0.5">
      ...
      <div className="flex justify-between items-center mb-4 pb-2.5 border-b border-terminal-dim">
        ...
        <div className="font-fira-code text-[10px] text-terminal-dim py-0.5 px-1.5 border border-terminal-dim rounded">{project.version}</div>
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-col gap-1 font-fira-code text-[10px] text-terminal-dim">
          <span className="project-owner">
            OWNER: <Link to={`/profile/${project.owner}`} className="text-terminal-accent no-underline hover:text-shadow-[0_0_5px_var(--terminal-accent)]">{project.owner}</Link>
          </span>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-col gap-1.5">
          {project.activity?.slice(0, 2).map((activity, index) => (
            <div key={index} className="flex flex-col gap-0.5 p-1.5 bg-[rgba(0,17,0,0.2)] border border-terminal-dim rounded font-fira-code text-[9px]">
              <span className="text-terminal-accent font-bold">{activity.user}</span>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button className={`${activityButtonStyle('var(--terminal-accent)')} flex-1 min-w-[100px]`}>
          DOWNLOAD
        </button>
        {project.checkoutStatus === 'checked-in' && (
          <button className={`${activityButtonStyle('var(--terminal-warning)')} flex-1 min-w-[100px] border-terminal-warning text-terminal-warning`}>
            CHECKOUT
          </button>
        )}
      </div>
